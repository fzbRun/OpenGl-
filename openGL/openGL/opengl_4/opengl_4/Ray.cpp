...
std::vector<MyObject*>* Ray::hitScene(MyList* hit) {
	...
	if (this->hitBox(hit->box,0)) {
		...
		if (hit->objects->size() <= 2 || hit->nextLeftNode == NULL) {
			...
			//如果不是叶子节点
			std::vector<MyObject*>* result;
			MyList* left = hit->nextLeftNode;
			if ((result = hitScene(left)) != NULL) {
				return result;
			}

			MyList* right = hit->nextRightNode;
			if ((result = this->hitScene(right)) != NULL) {
				return result;
			}
		}
...
vec4 Ray::getBandD(Scene* scene,std::vector<vec3*> lights) {
	...
	std::vector<MyObject*>* hitObjects = this->hitScene( &(scene->bvhObject) );
	...
	if (hitObjects == NULL) {
		...
		for (MyObject* object : *hitObjects) {
		...
		if (point != vec3(0.0f)) {
			...
			if (prr < PRR) {
				...
				if (returnColor != vec4(0.0f)) {
					color += diffuse * returnColor * objectColor * 100.0f;
				}
			}
			...
			for (vec3* light : lights) {
				...
				color += objectColor * diffuse * lightAttenuation * 10.0f;//光强度为10.0f;
